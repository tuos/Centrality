...
void getCentNpartNcollSmeared(){
  ...
  // Open File
  TFile *infile = new TFile("./out/tables_Glauber2018B_ForEPOS5020Responsenbins200x_d20180730_eff99_SmearingEff.root");
...
*/
  ...
  TFile *ofile = new TFile("default_glauber_eff99smeared.root","recreate");
  ofile->cd();
  hNpartCent->Write();
  hNcollCent->Write();
  hTAACent->Write();
