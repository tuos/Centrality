...
void combineResponseHistWithGlauber(const int nbins = 200, const string label = "HFtowers", const string tags = "Glauber_Default_Para", int eff = 0) {
   ...
   const char* outFileName = "./out/tables_Glauber2018B_ForEPOS5020Responsenbins200x_d20180808_eff97_SmearingEff.root"; //effsys
...
if(SysEff1){ infilename = Form("%s/glauberPbPb_Default_1M.root",fdirname); eff=0; }
 ...
 ofstream txtfile("out/output_eff97_0808d.txt"); //effsys
 ...
 for(int i=0; i<nhist; i++) {
  //effhist_1 for 99%, effhist_2 for 97%, and effhist_3 for 101%; effhist_0 for 100%
  histEff[i] = (TH1D*)effFile->Get(Form("effSys/effhist_%d",i+2)); //effsys
 }
 ...
 for(unsigned int iev = 0; iev < Nevents; iev++) {
   ...
   if (rnd->Uniform(0,1) < histEff[eff]->GetBinContent(histEff[eff]->FindBin(parameter))){
     ...
     if(rnd->Uniform(0,1) < histEff[eff]->GetBinContent(histEff[eff]->FindBin(parameter)) -1){
       values.push_back(parameter);
     }
   }
...
//cout<<"parameter = "<<parameter<<endl;
 ...
 //effsys
      // This is for eff = 101%. When eff<100%, it should do nothing.
      if(rnd->Uniform(0,1) < histEff[eff]->GetBinContent(histEff[eff]->FindBin(parameter)) -1){
        hNpart->Fill(parameter,npart);
        hNcoll->Fill(parameter,ncoll);
        hNhard->Fill(parameter,nhard);
...
//cout<<"b"<<endl;
 ...
 TH1D* hEcc2Mean = (TH1D*)gDirectory->Get("hEcc2_1");
 TH1D* hEcc2Sigma = (TH1D*)gDirectory->Get("hEcc2_2");
 TH1D* hEcc3Mean = (TH1D*)gDirectory->Get("hEcc3_1");
 TH1D* hEcc3Sigma = (TH1D*)gDirectory->Get("hEcc3_2");
 ...
 nt->Write();
  ...
  for (int ibin = 0; ibin < nbins; ibin++) {
   ...
   TFile * outfile = new TFile("out/default_glauber_eff97smeared0808.root","recreate"); //effsys
   outfile->cd();
   ...
   TH1D *hBCent = new TH1D("hBCent","",200,0.0,200.0);
   TH1D *hEcc2Cent = new TH1D("hEcc2Cent","",200,0.0,200.0);
   TH1D *hEcc3Cent = new TH1D("hEcc3Cent","",200,0.0,200.0);
   for(int i = 0; i < nbins; i++){
      int ii = nbins-i;
      hNpartCent->SetBinContent(i+1, hNpartMean->GetBinContent(ii));
      hNcollCent->SetBinContent(i+1, hNcollMean->GetBinContent(ii));
      hTAACent->SetBinContent(i+1, hNcollMean->GetBinContent(ii)/67.6);
      hBCent->SetBinContent(i+1, hbMean->GetBinContent(ii));
      hEcc2Cent->SetBinContent(i+1, hEcc2Mean->GetBinContent(ii));
      hEcc3Cent->SetBinContent(i+1, hEcc3Mean->GetBinContent(ii));
   }
   hNpartCent->Write();
   hNcollCent->Write();
   hTAACent->Write();
