{
  ...
  TH1D *hist_A = new TH1D("hist_A","",203,0,203);
  hist_A->Sumw2();
  hist_A->SetLineColor(1);
  hist_A->SetLineWidth(1);
  tree->Project("hist_A","hiBin","HLT_HIMinBiasHfOrBSC_v1");

  TH1D *hist_B = new TH1D("hist_B","",203,0,203);
  hist_B->Sumw2();
  hist_B->SetLineColor(2);
  tree->Project("hist_B","hiBin","HLT_HIMinBiasHfOrBSC_v1&&pcollisionEventSelection");
  ...
  hist_B->Draw("same");
  TLegend *leg = new TLegend(0.16,0.76,0.4,0.92);
  leg->SetFillColor(10);
  leg->SetBorderSize(0.035);
  //leg->SetTextFont(42);
  leg->SetTextColor(1);
  leg->SetTextSize(0.04);
  leg->AddEntry(hist_Ac,"HLT_HIMinBiasHfOrBSC_v1","p");
  leg->AddEntry(hist_B,"HLT_HIMinBiasHfOrBSC_v1&&pcollisionEventSelection","p");
  leg->Draw();

  c1->SaveAs("hiBin_minbias.png");
}
