void plot(){
  ...
  c1->cd();
  TProfile *pEcc2Npart = new TProfile("pEcc2Npart","#epsilon_{n} vs Npart in 200 GeV CuAu;Npart;#epsilon_{n}",252,-0.5,251.5);
  nt_Cu_Au->Project("pEcc2Npart","Ecc2:Npart");
  pEcc2Npart->SetMarkerSize(1);
  pEcc2Npart->SetMarkerColor(2);
  pEcc2Npart->SetMarkerStyle(20);
  pEcc2Npart->SetLineColor(2);
  ...
  pEcc3Npart->SetMarkerColor(1);
  pEcc3Npart->SetMarkerStyle(25);
  pEcc3Npart->SetLineColor(1);
  pEcc3Npart->SetStats(0);
  pEcc3Npart->Rebin(rebins);
  pEcc3Npart->Draw("psame");
  TLegend *leg = new TLegend(0.6,0.7,0.88,0.86);
  leg->SetFillColor(10);
  leg->SetBorderSize(0);
  leg->SetTextFont(42);
  leg->SetTextSize(0.055);
  leg->AddEntry(pEcc2Npart,"#epsilon_{2}{part}","p");
  leg->AddEntry(pEcc3Npart,"#epsilon_{3}{part}","p");
  leg->Draw();
