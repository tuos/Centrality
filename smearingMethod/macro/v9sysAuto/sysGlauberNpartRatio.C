...
"Eff_Sys_1"};
 TFile *glauberFile = new TFile("histGlauberHydjet5020GeV0317x.root","read");
 ...
 double tmp;
for(int j=0; j<4; j++){
 histSys[j] = (TH1D*)histNpart[j]->Clone(Form("histNpart%d",j));
 ...
 TFile *effFile = new TFile("effSysNpart10Bins.root","read");
 TH1D *histNcollx;
 ...
 for(int i=0; i<nCentBin;i++){
  ...
  tmp+=(histNcollx->GetBinContent(i+1)-1)*(histNcollx->GetBinContent(i+1)-1);
 ...
 int markers[nSys]={7,2,5,24,25,26,27,28,30,31};
 for(int i=0; i<nSys; i++) {
...
//for(int i=0; i<nCentBin; i++) cout<<i*10<<" - "<<(i+1)*10<<"%   "<<(histSysAll->GetBinContent(i+1)-1)*100<<"%."<<endl;
  ...
  c1->SaveAs("npartSysValueRatio.pdf");
