...
for(int j=0; j<nCentBin; j++) cout<<histNcoll[0]->GetBinContent(j+1)<<", ";
 ...
 for(int i=0; i<nSys; i++) {
  ...
  histNcoll[i]->Draw("psame");
    ...
    for(int i=0; i<nSys; i++)
      leg0->AddEntry(histNcoll[i],Form("%s",sysV[i]),"p");
    leg0->Draw();
